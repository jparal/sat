...
/**
 * @file   platform.h
 * @brief  Platform checking in compile time (compiler and architecture).
 * See the perfect source of information at http://predef.sourceforge.net/
 * @author @jparal
 *
 * @revision{1.0}
 * @reventry{2007-03-05, @jparal}
 * @revmessg{Initial version}
 * @reventry{2008-06-10, @jparal}
 * @revmessg{Removed TODO about SIZEOF_VOID_P missing}
 */

#ifndef __SAT_PLATFORM_H__
#define __SAT_PLATFORM_H__
 ...
 * defined to a positive integral value which is unique to a given compiler
 * family or zero if the compiler is unrecognized
 *
 * \def PLATFORM_COMPILER_IDSTR
 ...
 * defined to an integral expression which is guaranteed to be monotonically
 * non-decreasing with increasing compiler versions. Will be zero for
 * unrecognized compilers.
 *
 * \def PLATFORM_COMPILER_VERSION_STR
 * A string representation of the compiler version, which may contain
 * additional info
 *
 * \def PLATFORM_COMPILER_VERSION_GT(maj,min,pat)
 * evaluate to non-zero if the compiler version greater-than provided version
 * \def PLATFORM_COMPILER_VERSION_GE(maj,min,pat)
 * evaluate to non-zero if the compiler version greater-or-equal provided
 * version
 * \def PLATFORM_COMPILER_VERSION_EQ(maj,min,pat)
 * evaluate to non-zero if the compiler version equal provided version
 * \def PLATFORM_COMPILER_VERSION_LE(maj,min,pat)
 * evaluate to non-zero if the compiler version less-or-equal  provided version
 * \def PLATFORM_COMPILER_VERSION_LT(maj,min,pat)
 * evaluate to non-zero if the compiler version less-than  provided version
 * \def PLATFORM_COMPILER_VERSION_IN(majlo,minlo,patlo,majhi,minhi,pathi)
 * evaluate to non-zero if the compiler version in the interval of provided
 * versions (including)
 *
 * \def PLATFORM_ARCH_32
 * 32-bit pointers defined to positive value if CPU is known to have
 * the indicated property, undef otherwise
 * \def PLATFORM_ARCH_64
 * 64-bit pointers defined to positive value if CPU is known to have
 * the indicated property, undef otherwise
 *
 ...
 * \def PLATFORM_ARCH_LITTLE_ENDIAN
...
#  define PLATFORM_COMPILER_VERSION_STR					\
  _STRINGIFY(__INTEL_COMPILER)"_"_STRINGIFY(_PLATFORM_INTEL_COMPILER_BUILD_DATE)
...
#define PLATFORM_COMPILER_VERSION_LT(maj,min,pat) \
  PLATFORM_COMPILER_VERSION <  PLATFORM_COMPILER_VERSION_INT(maj,min,pat)
#define PLATFORM_COMPILER_VERSION_IN(majlo,minlo,patlo,majhi,minhi,pathi) \
  PLATFORM_COMPILER_VERSION_GE(majlo,minlo,patlo) &&			\
  PLATFORM_COMPILER_VERSION_LE(majhi,minhi,pathi)
