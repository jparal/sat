...
#if defined(SAT_MEMORY_TRACKER) || defined(SAT_MEMORY_TRACKER_IMPLEMENT)

// #include "csextern.h"

/**
 * This structure is used per file to keep track of allocations.
 * MemTrackerModule maintains an array of them per module.
 */
struct // CS_CRYSTALSPACE_EXPORT
MemTrackerInfo
{
  char* file;
  size_t max_alloc;
...
namespace SAT
{
  namespace Debug
  {
    namespace MemTracker
    {
      namespace Impl
      {
	extern// CS_CRYSTALSPACE_EXPORT
	void RegisterAlloc (MemTrackerModule* m,
	  void* p, size_t s, const char* info);
	extern // CS_CRYSTALSPACE_EXPORT
	void RegisterModule (MemTrackerModule*& m, const char*);
	extern // CS_CRYSTALSPACE_EXPORT
	...
	extern MemTrackerModule* thisModule;
      } // namespace Impl
