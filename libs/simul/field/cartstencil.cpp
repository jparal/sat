			...
			Vector<T,3> &val)
{
  ...
  //   loc[i] -= 1; adj -= T(4)*fld (loc);
  //   if (loc[i] > 0) loc[i] -= 1; else loc[i] += 3;
  //   adj += fld (loc);

  //   loc = iter.GetLoc ();
  //   loc[i] += 1; adj -= T(4)*fld (loc);
  //   if (loc[i] < fld.GetSize (i)-1) loc[i] += 1; else loc[i] -= 3;
  //   adj += fld (loc);

  //   T dxi = fld.GetMesh().GetResolInv (i);
  //   val +=  dxi*dxi*dxi*dxi * adj;
  // }
