...
; PURPOSE:
;
;    The purpose of this function is to have a device-independent error
;    messaging function. The error message is reported to the user by using
;    DIALOG_MESSAGE if widgets are supported and MESSAGE otherwise.
;
;    In general, the ERROR_MESSAGE function is not called directly.  Rather, it
;    is used in a CATCH error handler. Errors are thrown to ERROR_MESSAGE with
;    the MESSAGE command. A typical CATCH error handler is shown below.
;
;       Catch, theError
...
; INPUTS:
;
;    the_Error_Message: This is a string argument containing the error message
;       you want reported. If undefined, this variable is set to the string in
;       the !Error_State.Msg system variable.
;
; KEYWORDS:
;
;    ERROR: Set this keyword to cause Dialog_Message to use the ERROR reporting
;       dialog. Note that a bug in IDL causes the ERROR dialog to be used
;       whether this keyword is set to 0 or 1!
;
;    INFORMATIONAL: Set this keyword to cause Dialog_Message to use the
;       INFORMATION dialog instead of the WARNING dialog. Note that a bug in
;       IDL causes the ERROR dialog to be used if this keyword is set to 0!
;
...
;
;    TRACEBACK: Setting this keyword results in an error traceback being
;       printed to standard output with the PRINT command. Set to 1 (ON) by
;       default. Use TRACEBACK=0 to turn this functionality off.
;
; OUTPUTS:
;
