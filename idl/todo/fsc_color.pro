...
; PURPOSE:
;
;       The purpose of this function is to obtain drawing colors by name and in
;       a device/decomposition independent way.  The color names and values may
;       be read in as a file, or 104 color names and values are supplied with
;       the program. These colors were obtained from the file rgb.txt, found on
;       most X-Window distributions.  Representative colors were chosen from
;       across the color spectrum. To see a list of colors available, type:
;
...
;       the same code will work in all graphics devices I have tested.  These
;       include the PRINTER, PS, and Z devices, as well as X, WIN, and MAC.
;
;       In practice, graphics code is seldom written like this. (For a variety
;       of reasons, but laziness is high on the list.) So I have made the
;       program reasonably tolerant of poor programming practices. I just point
;       this out as a place you might return to before you write me a nice note
;       saying my program "doesn't work". :-)
;
...
;
;       theColor: A string with the "name" of the color. To see a list of the
;           color names available set the NAMES keyword. This may also be a
;           vector of color names. Colors available are these:
;
;           Active Almond Antique White Aquamarine Beige Bisque Black Blue Blue
;       Violet Brown Burlywood Cadet Blue Charcoal Chartreuse Chocolate Coral
;       Cornflower Blue Cornsilk Crimson Cyan Dark Goldenrod Dark Gray Dark
;       Green Dark Khaki Dark Orchid Dark Red Dark Salmon Dark Slate Blue Deep
;       Pink Dodger Blue Edge Face Firebrick Forest Green Frame Gold Goldenrod
;       Gray Green Green Yellow Highlight Honeydew Hot Pink Indian Red Ivory
;       Khaki Lavender Lawn Green Light Coral Light Cyan Light Gray Light
;       Salmon Light Sea Green Light Yellow Lime Green Linen Magenta Maroon
;       Medium Gray Medium Orchid Moccasin Navy Olive Olive Drab Orange Orange
;       Red Orchid Pale Goldenrod Pale Green Papaya Peru Pink Plum Powder Blue
;       Purple Red Rose Rosy Brown Royal Blue Saddle Brown Salmon Sandy Brown
;       Sea Green Seashell Selected Shadow Sienna Sky Blue Slate Blue Slate
;       Gray Snow Spring Green Steel Blue Tan Teal Text Thistle Tomato
;       Turquoise Violet Violet Red Wheat White Yellow
;
;           The color WHITE is used if this parameter is absent or a color name
;           is mis-spelled. To see a list of the color names available in the
;           program, type this:
;
;              Print, FSC_Color(/Names), Format='(6A18)'
;
;       theColorIndex: The color table index (or vector of indices the same
;           length as the color name vector) where the specified color is
;           loaded. The color table index parameter should always be used if
;           you wish to obtain a color value in a
;           color-decomposition-independent way in your code. See the NORMAL
;           CALLING SEQUENCE for details. If theColor is a vector, and
;           theColorIndex is a scalar, then the colors will be loaded starting
;           at theColorIndex.
;
; RETURN VALUE:
;
;       The value that is returned by FSC_Color depends upon the keywords used
;       to call it, on the version of IDL you are using,and on the depth of the
;       display device when the program is invoked. In general, the return
;       value will be either a color index number where the specified color is
;       loaded by the program, or a 24-bit color value that can be decomposed
;       into the specified color on true-color systems. (Or a vector of such
;       numbers.)
;
...
;       the program is called. If you are running a version of IDL before IDL
;       5.2, then the program will return the color index number. This behavior
;       can be overruled in all versions of IDL by setting the DECOMPOSED
;       keyword.  If this keyword is 0, the program always returns a color
;       index number. If the keyword is 1, the program always returns a 24-bit
;       color value.
;
;       If the TRIPLE keyword is set, the program always returns the color
;       triple, no matter what the current decomposition state or the value of
;       the DECOMPOSED keyword. Normally, the color triple is returned as a 1
;       by 3 column vector.  This is appropriate for loading into a color index
;       with TVLCT:
;
...
;
;       If the ALLCOLORS keyword is used, then instead of a single value,
;       modified as described above, then all the color values are returned in
;       an array. In other words, the return value will be either an
;       NCOLORS-element vector of color table index numbers, an NCOLORS-element
;       vector of 24-bit color values, or an NCOLORS-by-3 array of color
;       triples.
;
;       If the NAMES keyword is set, the program returns a vector of color
;       names known to the program.
;
;       If the color index parameter is not used, and a 24-bit value is not
;       being returned, then colorIndex is typically set to
;       !D.Table_Size-1. However, this behavior is changed on 8-bit devices
;       (e.g., the PostScript device, or the Z-graphics buffer) and on 24-bit
;       devices that are *not* using decomposed color. On these devices, the
;       colors are loaded at an offset of !D.Table_Size - ncolors - 2, and the
;       color index parameter reflects the actual index of the color where it
;       will be loaded. This makes it possible to use a formulation as below:
;
;          Plot, data, Color=FSC_Color('Dodger Blue')
;
;       on 24-bit displays *and* in PostScript output! Note that if you specify
;       a color index (the safest thing to do), then it will always be honored.
;
; INPUT KEYWORD PARAMETERS:
;
;       ALLCOLORS: Set this keyword to return indices, or 24-bit values, or
;              color triples, for all the known colors, instead of for a single
;              color.
;
...
;
;       FILENAME: The string name of an ASCII file that can be opened to read
;              in color values and color names. There should be one color per
;              row in the file. Please be sure there are no blank lines in the
;              file.  The format of each row should be:
;
;                  redValue  greenValue  blueValue  colorName
;
;              Color values should be between 0 and 255. Any kind of
;              white-space separation (blank characters, commas, or tabs) are
;              allowed. The color name should be a string, but it should NOT be
;              in quotes. A typical entry into the file would look like this:
;
;                  255   255   0   Yellow
;
;       NAMES: If this keyword is set, the return value of the function is a
;              ncolors-element string array containing the names of the colors.
;              These names would be appropriate, for example, in building a
;              list widget with the names of the colors. If the NAMES keyword
;              is set, the COLOR and INDEX parameters are ignored.
;
;                 listID = Widget_List(baseID, Value=GetColor(/Names), YSize=16)
;
;       ROW:   If this keyword is set, the return value of the function when
;              the TRIPLE keyword is set is returned as a row vector, rather
;              than as the default column vector. This is required, for
;              example, when you are trying to use the return value to set the
;              color for object graphics objects. This keyword is completely
;              ignored, except when used in combination with the TRIPLE
;              keyword.
;
;       SELECTCOLOR: Set this keyword if you would like to select the color
;              name with the PICKCOLORNAME program. Selecting this keyword
;              automaticallys sets the INDEX positional parameter. If this
;              keyword is used, any keywords appropriate for PICKCOLORNAME can
;              also be used. If this keyword is used, the first positional
;              parameter can be either a color name or the color table index
;              number. The program will figure out what you want.
;
;       TRIPLE: Setting this keyword will force the return value ofq the
;              function to *always* be a color triple, regardless of color
;              decomposition state or visual depth of the machine. The value
;              will be a three-element column vector unless the ROW keyword is
;              also set.
;
;       In addition, any keyword parameter appropriate for PICKCOLORNAME can be
;       used.  These include BOTTOM, COLUMNS, GROUP_LEADER, INDEX, and TITLE.
;
; OUTPUT KEYWORD PARAMETERS:
;
;       CANCEL: This keyword is always set to 0, unless that SELECTCOLOR
;              keyword is used.  Then it will correspond to the value of the
;              CANCEL output keyword in PICKCOLORNAME.
;
;       COLORSTRUCTURE: This output keyword (if set to a named variable) will
;              return a structure in which the fields will be the known color
;              names (without spaces) and the values of the fields will be
;              either color table index numbers or 24-bit color values. If you
;              have specified a vector of color names, then this will be a
;              structure containing just those color names as fields.
;
;       NCOLORS: The number of colors recognized by the program. It will be 104
;       by default.
;
; COMMON BLOCKS:
;       None.
