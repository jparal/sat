;--------------------------------------------------------------------;
; BASIC PROCEDURE FOR PLOTTING DENSITY DATA                          ;
...
;                                                                    ;
;    IDL> pl_fign,mina=0.95,maxa=1.1,radius=1.25,                    ;
...
if (ss(1) eq 0) then begin
  rd,dir+'/'+'Dn'+specie+run+itt+'.gz',dn
  ss=size(dn)
endif
...
if not(keyword_set(mina)) then mina=min(dn)
if not(keyword_set(maxa)) then maxa=max(dn)
