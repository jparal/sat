 ...
 if (type eq 'WIDGET_BUTTON') then begin
    widget_control, get_value=i , event.id  
      ...
      if (i eq 'ener') then begin
        rh0, u,cdn, cux, cuy, cuz,cpe,cpx,cpy,cpz,cpxy,cpxz,csx ,ff 
        !p.title='R-H energy'
        !p.subtitle=' u = '+string(u)
      endif  
      ...
      if ((i eq ' DRAW ') or (i eq ' PRINT ')) then begin
       if (i eq ' PRINT ') then begin
        if(pslog eq 0) then begin
         pslog=1
         set_plot,'ps'
         device,/color,/landscape, filename=filnam
        endif
       endif else begin
        pslog=0
        set_plot,'x'
...
;           if (a eq 7) then image, sign*fft( ff(*,*,y,z),1)
          ...
          if (b eq 1) then begin
           ...
           if (a eq 8) then image_cont1,fft(sign*reform(ff(*,*,z)),1)
          ...
          if (b eq 2) then begin
           ...
           if (a eq 8) then image_cont1,sign*fft(reform(ff(x,*,*)),1)
          ...
          if (b eq 5) then begin
           ...
           if (a eq 8) then image_con1t,sign*fft(reform(ff(*,y,*)),1)
