...
; and shock solution of given theta and vsh /kmax=10
pro anex,ikpe,ikpa,omer,omei,theta,vsh,ki,om,kip,omp,ai0,ai1,ai2,ai3,ai4 
kmax=10.
s=size(omei)
n1=s(1)
n2=s(2)
...
for k=0,nn(1)-1 do begin
 if (abs(imaginary(roots(k))) lt 1.e-6)then begin
  if (float(roots(k)) lt kmax) then begin
  kin=max([kin,float(roots(k))])
  endif
 endif
endfor
ki(i,j)=kin
