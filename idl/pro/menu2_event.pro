;
  pro menu2_event, event
  common d3fields, cbx,cby,cbz,$
                    cex,cey,cez,$
                    cux,cuy,cuz,$
                    cdn,cpe,$ 
                    cpx,cpy,cpz,$
                    cpxy,cpxz,cpyz,cpxxx,cpxyz
  common ran, u
...
;111111111111111111111111111111111111111111111111111111111111111111111111111
 if (type eq 'WIDGET_BUTTON') then begin
    widget_control, get_value=i , event.id  
      ...
      if (i eq 'px') then begin
       ff = cpx 
       !p.title=' Graf !17 p!Ixx!3 '
      endif
      if (i eq 'py') then begin
       ff = cpy 
       !p.title=' Graf !17 p!Iyy!3 '
      endif 
      if (i eq 'pz') then  begin
       ff = cpz 
       !p.title=' Graf !17 p!Izz!3 '
      endif    
      if (i eq 'qx') then  begin
       ff = 0.5*( cpxxx+cpxyz)
       !p.title=' Graf !17 q!Ix!3 '
      endif    
      if (i eq 'pxy') then begin
       ff = cpxy 
       !p.title=' Graf !17 p!Ixy!3 '
      endif
      if (i eq 'pxz') then begin
       ff = cpxz 
       !p.title=' Graf !17 p!Ixz!3 '
      endif 
      if (i eq 'pyz') then  begin
       ff = cpyz 
       !p.title=' Graf !17 p!Iyz!3 '
      endif    
      if (i eq 'pxyz') then begin
       ff = cpxyz 
       !p.title=' Graf !17 p!Ixyy!3 + p!Ixzz!3 '
      endif 
      if (i eq 'pxxx') then  begin
       ff = cpxxx 
       !p.title=' Graf !17 p!Ixxx!3 '
      endif    
      ...
      if (i eq 'ener') then begin
        rh0, u,cdn, cux, cuy, cuz,cpe,cpx,cpy,cpz,cpxy,cpxz, $
        cpxxx,cpxyz,csx ,ff 
        !p.title='R-H energy'
        !p.subtitle=' u = '+string(u)
      endif  
      if (i eq 'bn') then begin
        rh6, u,cdn, cux, cuy, cuz,cpe,cpx,cpy,cpz, cbx, cby, cbz ,ff 
        !p.title='R-H B normal'
        !p.subtitle=' u = '+string(u)
      endif  
      if (i eq 'et1') then begin
        rh7, u,cdn, cux, cuy, cuz,cpe,cpx,cpy,cpz,cbx, cby, cbz ,ff 
        !p.title='R-H E tang1'
        !p.subtitle=' u = '+string(u)
      endif  
      if (i eq 'et2') then begin
        rh8, u,cdn, cux, cuy, cuz,cpe,cpx,cpy,cpz, cbx, cby, cbz ,ff 
        !p.title='R-H E tang2'
        !p.subtitle=' u = '+string(u)
      endif        
      ...
      if (i eq '1x4') then !p.multi=[0,0,4,0,0]
...
;           if (a eq 9) then velovect, ff(*,*,y,z),fff(*,*,y,z)
          ...
          if (b eq 1) then begin
           ...
           if (a eq 8) then im,sign*reform(ff(*,*,z))
          ...
          if (b eq 2) then begin
           ...
           if (a eq 8) then im,sign*reform(ff(x,*,*))
          ...
          if (b eq 5) then begin
           ...
           if (a eq 8) then im,sign*reform(ff(*,y,*))
           if (a eq 7) then image,sign*reform(ff(*,y,*))
           if (a eq 7) then velovect,reform(ff(*,y,*)),reform(fff(*,y,*))
          endif
...
;           if (a eq 8) then im,sign*reform(ff(*,x,y,*))
...
;           if (a eq 8) then im,sign*reform(ff(*,x,*,z))
...
;44444444444444444444444444444444444444444444444444444444444444444444
 ...
 ;        if (c eq 0) then plot,fft(sign*ff(*,x,y,z),1)
         if (c eq 1) then plot,fft(sign*ff(*,y,z),1)
         if (c eq 2) then plot,fft(sign* ff(x,*,z),1)
         if (c eq 3) then plot,fft(sign* ff(x,y,*),1)
        endif else begin 
;5555555555555555555555555555555555555555555555555555555555555555555
         if (a eq 4) then begin
;          if (c eq 0) then oplot,fft(sign* ff(*,x,y,z),1)
          if (c eq 1) then oplot,fft(sign*  ff(*,y,z),1)
          if (c eq 2) then oplot,fft(sign* ff(x,*,z),1)
          if (c eq 3) then oplot,fft(sign* ff(x,y,*),1)
         endif else begin
          if (b eq 0) then begin
...
;;           if (a eq 8) then im,sign*fft( ff(*,*,y,z),1)
;           if (a eq 7) then image, sign*fft( ff(*,*,y,z),1)
          endif
          if (b eq 1) then begin
           if (a eq 0) then contour,sign*fft(reform(ff(*,*,z)),1)
           if (a eq 1) then surface,sign*fft(reform(ff(*,*,z)),1)
           if (a eq 2) then show3,sign*fft( reform(ff(*,*,z)),1)
           if (a eq 5) then shade_surf,fft(sign*reform(ff(*,*,z)),1)
           if (a eq 6) then image_cont,fft(sign*reform(ff(*,*,z)),1)
           if (a eq 8) then im,fft(sign*reform(ff(*,*,z)),1)
           if (a eq 7) then image,sign*fft(reform(ff(*,*,z)),1)
          endif
          if (b eq 2) then begin
           if (a eq 0) then contour,sign*fft(reform(ff(x,*,*)),1)
           if (a eq 1) then surface,sign*fft(reform( ff(x,*,*)),1)
           if (a eq 2) then show3,sign*fft(reform(ff(x,*,*)),1)
           if (a eq 5) then shade_surf,sign*fft(reform(ff(x,*,*)),1)
           if (a eq 6) then image_cont,sign*fft(reform(ff(x,*,*)),1)
           if (a eq 8) then im,sign*fft(reform(ff(x,*,*)),1)
           if (a eq 7) then image,sign*fft(reform(ff(x,*,*)),1)
          endif
          if (b eq 5) then begin
           if (a eq 0) then contour,sign*fft(reform(ff(*,y,*)),1)
           if (a eq 1) then surface,sign*fft(reform( ff(*,y,*)),1)
           if (a eq 2) then show3,sign*fft(reform(ff(*,y,*)),1)
           if (a eq 5) then shade_surf,sign*fft(reform(ff(*,y,*)),1)
           if (a eq 6) then image_cont,sign*fft(reform(ff(*,y,*)),1)
           if (a eq 8) then im,sign*fft(reform(ff(*,y,*)),1)
           if (a eq 7) then image,sign*fft(reform(ff(*,y,*)),1)
          endif
          if (b eq 4) then begin
...
;           if (a eq 8) then im,sign*fft(reform(ff(*,x,y,*)),1)
...
;           if (a eq 8) then im,sign*fft(reform(ff(*,x,*,z)),1)
