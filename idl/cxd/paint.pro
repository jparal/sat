...
;-------------------------------------------------------------
	...
	;-------  Options  ----------------
	val = ['Q','H','P','D','FF','RC','PF','PC','IC','S','R','R0']
...
'RC':	begin		; Help
	  ...
	  wset, win1
	  wshow
	  crossi,ix,iy,/dev,/dia,/mag,menu=['Replace color','Cancel'],exit=ex
	  ...
	  if ex eq 0 then begin
	    ...
	    if n_elements(bound) eq 0 then begin
	      xhelp, ['Flood filling region . . .'], $
	        /nowait,wid=wid
	      seedfillr, work, seed=[ix,iy],fill=clr
	      widget_control,wid,/dest
	      wid = 0
	    endif else begin
	      xhelp, ['Flood filling region . . .'], $
	        /nowait,wid=wid
	      seedfill, work, seed=[ix,iy],fill=clr, bound=bound
	      widget_control,wid,/dest
	      wid = 0
	    endelse
	    tv,work
