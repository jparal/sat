...
; OUTPUTS:
; COMMON BLOCKS:
;       ps_com,dname,xthick,ythick,pthick,pfont,defnum,num,comm,pfil
; NOTES:
...
;-------------------------------------------------------------
	...
	common ps_com,dname,xthick,ythick,pthick,pfont,defnum,num,comm,pfile
	...
	if keyword_set(nop) then begin
	  if not keyword_set(nostmp) then begin
	    xyouts,-100,0,/dev,orient=90,size=.4,getenv('USER')+' '+$
	      systime(0) + ' ' + comm
	  endif
	  device, /close
...
skip:	if qflg then print,' Sending plot to postscript printer . . .'
	if not keyword_set(nostmp) then begin
	  xyouts,-100,0,/dev,orient=90,size=.4,getenv('USER')+' '+systime(0)+$
	    ' ' + comm
	endif
