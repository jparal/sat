...
'JPG':	begin
	  read_jpeg, f, a, ct, colors=!d.n_colors-1, /dither, /two_pass
	  r = ct(*,0)
	  g = ct(*,1)
	  b = ct(*,2)
	end
...
;===================================================================
	...
	for i=0, n-1 do begin
	  ...
	  yy0 = sf*m.py0(i)
	  xx = xx0>0<(nx-dx(i))	; Keep in bounds.
	  yy = yy0>0<(ny-dy(i))
...
;===================================================================
	...
	if !d.n_colors ge 216 then cube=6 else cube=5
	t = bytarr(5,5)
...
;===================================================================
	...
	if cmd eq 'ADDTIF' then begin
	  ...
	  xtxtin,t,titl='Enter index of transparent color (none=-1)',$
	    /wait,def='-1',menu=['0',strtrim(!d.n_colors-1,2)]
	...
	if cmd eq 'TEXT' then begin
	  ...
	  if out ne '' then begin
	    my = 2.56*m.sc_y/scale
	    x0 = fix(x/sf)
	    y0 = my - (fix(y/sf)+ny-1)
	    tin = out+' '+strtrim(x0,2)+' '+strtrim(y0,2)
	    xmer_add, tin, m
	  endif
	  widget_control, ev.id, /input_focus
	  return
...
;	id = widget_button(b, value='Xview',uval='XVIEW')
	...
	map = {top:top, genwin:0, $
	  ...
	  ptyp:strarr(20), rbk:255, gbk:255, bbk:255, bk:!d.n_colors-1, $
