...
;-------------------------------------------------------------
	...
	stat = keyword_set(nostat) eq 0
	  ...
	  y = y>0<1.
          ...
          yydv = yydv(sort(yydv))
          if n_elements(x) eq 0 then y = total(yy)/2.
	  y = y>yy(0)<yy(1)
          x = total(xx)/2.
        endelse
	...
	xdv = tmp(0)<xxdv(1)  & ydv = tmp(1)<yydv(1)
        ...
        if not keyword_set(xmode) then t=tvrd(0,ydv,!d.x_size,1) ; 1st row.
	...
	;===========================================================
	  ...
	  ;------  Update line  --------------
          ...
          if not keyword_set(xmode) then begin
            tv, t, 0, yl                          ; Replace last row. 
            t = tvrd(0,ydv,!d.x_size,1)           ; Read new column.
          endif else begin
