...
;         /NOEXTEND means inhibit automatic size extension of
;           32 pixels.  Window is that much smaller than requested.
; OUTPUTS:
; COMMON BLOCKS:
;       swindow_com
; NOTES:
...
;-------------------------------------------------------------
	...
	pro swindow, index=indx, colors=colors, $
	  xsize=xsize, ysize=ysize, x_scroll_size=xscr, $
	  y_scroll_size=yscr, title=title, noextend=noextend, help=hlp
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   /NOEXTEND means inhibit automatic size extension of'
	  print,'     32 pixels.  Window is that much smaller than requested.'
	...
	add = 32					; Handle window extend.
	if keyword_set(noextend) then add = 0
	...
	t = widget_draw(b,xs=xsize+add,ys=ysize+add,x_scr=xscr+add,$
	  y_scr=yscr+add, colors=colors, retain=2)
