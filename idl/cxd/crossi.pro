...
;-------------------------------------------------------------
        ...
        pro crossi, x, y, color=color, linestyle=linestyle, $ 
	  ...
	  xsize=xsize, ysize=ysize, dialog=dialog,menu=menu,$
          instructions=instr, js=js, mag=mag0, size=msize0
...
;	  wset, win1					; Return to first win.
	...
	xcl = -2  & ycl = -2
	  ...
	  if mag0 ne 0 then begin
	    ...
	    if st.lum(it) lt 128 then cc=st.bright $
	       else cc=st.dark  		    ; Outline color.
	    ...
	    wset, st.win2			    ; Set to mag window.
	...
	y = tmp(1)
	  ...
	  if mag0 ne 0 then wdelete, st.win2
        endif
