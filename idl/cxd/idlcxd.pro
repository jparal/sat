...
;*************************************************
PRO idlcxd;,ii,hour,mins,nmins,hicut,lowcut
...
array_name_index=5;4;5;1;5;1;2;4;2;1;4;1;3  ;*****change 1
...
print, event
name_file= 'pbq20010329d.min';'19900728FSIM.MAG';G8_K0_CGM_SC.txt';far2000071600.col';G0_K0_MAG_270301.txt';pbq20010327d.min'; '20010327GILL.MAG';image2001032700.col2';kev2001032700.col';pbq20010329d.min';20010329GILL.MAG';G8_K0_MAG_290301.txt';'pbq20010329d.min' ;20010329GILL.MAG';'bfac2708.txt';20010327FCHU.MAG';20010329GILL.MAG';image2001032900.col2';20000716_1991-080.sopaflux';naq20000716d.min';kil2000071600.col';far20000716.txt';naq20000716d.min';far20000716.txt';G8_K0_CGM_SC.txt';20000716_1991-080.sopaflux';far20000716.txt';20000716_1991-080.sopaflux';kev2000071600.col';20000716GILL.MAG';20000716_1994-084.sopaflux';20000716_1989-046.sopaflux';kev2000071600.col';20000716_1989-046.sopaflux';20000716_1994-084.sopaflux';kil2000071600.col';far20000716.txt';20000716_1989-046.sopaflux';1994-084.sopaflux';a5000716.zyk';kil2000071600.col';naq20000716d.min';a5000716.zyk';kil20000716.txt';20000716_1991-080.sopaflux';far20000716.txt';20000716ESKI.MAG';kev2000071600.col';G8_K0_CGM_SC.txt';20000716_1994-084.sopaflux';cmo20000716d.min';20000716_1989-046.sopaflux';20000716_1994-084.sopaflux';a5000716.zyk';20000716_1994-084.sopaflux';a5000716.zyk';zyk20000716.txt';20000716_1991-080.sopaflux';20000716GILL.MAG';G8_K0_CGM_SC.txt';20000716_1991-080.sopaflux';zyk20000716.txt';20000716ISLL.MAG';kil2000071600.col';G8_K0_CGM_SC.txt';MAG_15835.txt';irt20000716d.min';20000716.txt_1994-084.sopaflux';19900728GILL.MAG';kil1990072800.col';20000716_1994-084.sopaflux';G0_K0_MAG_10198.txt';lyc2000071600.col';20000716GILL.MAG';G8_K0_CGM_SC.txt';20000609_1994-084.sopaflux';20000716DAWS.MAG';kev2000071600.col';20000716_1994-084.sopaflux';bor20000716.txt' ;*****change 5
Points=1440;17280;8640;1440;17280;1440;17280;1440;=17280;1440;8640;1400;8640;0;1440;86400;1440;6285;86400;6285;8640;17280;8606;8640;8065;8606;8640; 8065;8606; 1440;86400;6285;86400.;17280;8640;1440;8606;1440;8065;8606;6285;17280;1440; 6285;1440;8606;6285;1440;8469;606 ;****change 6
data_path='~/work/data/'+array+'/'+event+'/'
data_file = data_path+name_file

lnum1 = 102.
print, data_file
case strupcase(strtrim(array_name,1)) of
 'CANOPUS' :Xstatn=utl_input_canopus(data_file)
 'IMAGE' :Xstatn=utl_input_image(data_file,points)
 'SAMNET':Xstatn=utl_input_samnet(data_file)
 'GOES' :xstatn=utl_input_goes(data_file,Points);Xstatn=gsm2fac(data_file,lnum1);utl_input_goes(data_file,Points)
 'LANL':Xstatn= utl_input_lanl(data_file,Points)
 'INTERMAG' : Xstatn=utl_input_intermag(data_file); utl_input_intermag(data_file);,Points)	;210(data_file,Points);intermag(data_file,Points)	
endcase

; data3 =  gsm2fac(filename3,lnum1)
; gx = data3.bx_fac
; gy = data3.by_fac
; gz = data3.bz_fac
; gtime = data3.b_time
; g_t = sqrt(gx*gx + gy*gy + gz*gz)
...
; plot,TimeFile/60./60.,Xstatn.by_fac,XStyle=1,xticks=3,ystyle=1,xtitle ='Time (UT)',$
;plot,TimeFile/60./60.,Xstatn.Datafile[jj,*],XStyle=1,xticks=3,ystyle=1,xtitle ='Time (UT)',$
; plot,TimeFile/60./60.,Xstatn.X4,XStyle=1,xticks=3,ystyle=1,xtitle ='Time (UT)',$
...
; h = Xstatn.bx_fac
; d = Xstatn.by_fac
	h=Xstatn.dataFile[0,*];a.data_vals.h
	d=Xstatn.dataFile[1,*];a.data_vals.d
endif else begin
ii = 3
Sint = Xstatn.Sint    ;float - Data sample rate
DataFile=Xstatn.Ydatafile[ii,*]  ;float array - Magnetic Data
Points=Xstatn.Points          ;long array - total number of data points
h=dataFile/mean(dataFile);a.data_vals.h
...
endelse
	...
	datum1=LONG(hour*60*60/T + mins*60/T)
	...
	; Prepare output file
	OpenW, lun, 'idlcxd2.txt', /GET_LUN, /APPEND
	PrintF, lun, 'Running Complex Demodulation on ', data_file
	PrintF, lun, FORMAT='("Start time ", (I2.2), ":", (I2.2)," UT  Duration: ", I6)', hour, mins, nmins
...
;	print, 'fsd= ', fsd
	...
	line2=STRING(FORMAT='("Start time ", (I2.2), ":", (I2.2)," UT")', hour, mins)
