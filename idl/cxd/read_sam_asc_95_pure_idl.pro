...
;+
; NAME:
;       read_sam_asc_95_pure_idl
; PURPOSE:
;       Read SAMNET data from a 1 second format (1995 format) ASCII file.
; CATEGORY:
; CALLING SEQUENCE:
;	datafile=read_sam_asc_95_pure_idl(filename)
; INPUTS:
...
;-------------------------------------------------------------
	...
	function read_sam_asc_95_pure_idl, filename, $
	...
	if n_elements(filename) eq 0 then filename="    no filename set"
        ...
        if keyword_set(hlp) then $
          ...
          print,''
	  print,' NAME:'
	  print,'       read_sam_asc_95_pure_idl'
	  print,' PURPOSE:'
	  print,'       Read SAMNET data from a 1 second format (1995) ASCII file.'
	  print,' CATEGORY:'
	  print,' CALLING SEQUENCE:'
	  print,'	datafile=read_sam_asc_95_pure_idl(filename)'
	  print,' INPUTS:'
...
;	format='(A1,A1,A1,1X,i2,1x,2i2,1x,3i2,2x,i1)'
	...
	readf,in_unit,stn1,stn2,day,month,year,hour,min,sec,type_test,$
	format='(A1,A1,i2,1x,i2,1x,i4,1x,i2,i2,i2,1x,a2,27x)'

;	Assume a whole day file
	num_mins=1440
	sample_interval=1.0
	if type_test eq '05' then begin
		sample_interval=5.0
	endif
	num_points=86400/sample_interval
...
;	Might get some data back even if we reach EOF
	...
	hdzvals=fltarr(3,num_points)
;print,systime(),'reading arrays'
	if sample_interval eq 1.0 then begin
		readf,in_unit,hdzvals,format='(/,60(3f7.1,:,/))'
	endif
	if sample_interval eq 5.0 then begin
		readf,in_unit,hdzvals,format='(/,720(3f8.2,:,/))'
	endif
;print,systime(),'reforming data'
	makedata:
