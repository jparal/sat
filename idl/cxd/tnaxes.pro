...
;	R. Bunting, 15 Jul, 1996 --- modified to honour !x.minor
...
;-------------------------------------------------------------
	...
	IF (N_PARAMS(0) LT 6) or (keyword_set(hlp)) THEN BEGIN
	  ...
	  PRINT,' tnaxes, xmn, xmx, nx, mjx1, mjx2, xinc, [mnx1, mnx2, xinc2]
...
DONE:	if !x.minor ne 0 then begin xi2 = xi / !x.minor & endif
	IF DX LE 0. THEN begin XI = -XI & xi2 = -xi2 & endif
