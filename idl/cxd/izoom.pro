...
;-------------------------------------------------------------
	...
	pro izoom, x0, y0, z0, xrange=xran0, yrange=yran0, interp=interp, $
	  js=js, offset=off, _extra=extra, help=hlp
	...
	x = [x0,x0(nx-1)+dx]		; Extended X array.
	y = [y0,y0(ny-1)+dy]		; Extended Y array.
	sz = size(z0)			; Z array size.
	z = make_array(sz(1)+1,sz(2)+1,type=sz(3))	; New Z array.
	z(0,0) = z0			; Embed old Z.
	z(nx,*) = z(nx-1,*)		; Copy rows/columns.
	z(*,ny) = z(*,ny-1)
	
	;---------  Handle defaults  ------------
	if n_elements(interp) eq 0 then interp=0	; interp default.
	xmn = min(x0,max=xmx)				; Full ranges from
	ymn = min(y0,max=ymx)				;   original arrays.
	...
	imgunder, z(xlo:xhi,ylo:yhi), interp=interp
