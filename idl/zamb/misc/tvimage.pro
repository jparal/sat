...
;
;     INTERP:   Setting this keyword enables the bilinear interpolation
;               to be done to the image when it is resized. Nearest
;               neighbor interpolation is the default. This is preferred
...
;      Changed the NOINTERPOLATION keyword to INTERP (and inverted the meaning),
;      fixed the !P.Multi behaviour 23 Sep 2000. zamb.
;-
;
;###########################################################################
;
...
PRO TVIMAGE, image, x, y, KEEP_ASPECT_RATIO=keep, POSITION=position, $
   ...
   HALF_HALF=half_half, BACKGROUND=background, $
   INTERP = interp
...
IF N_ELEMENTS(position) EQ 0 THEN BEGIN
   IF Keyword_Set(multi) AND (Keyword_Set(overplot) NE 1) THEN BEGIN
      ...
      position = [!X.Region[0], !Y.Region[0], !X.Region[1], !Y.Region[1]]
   ENDIF ELSE BEGIN
