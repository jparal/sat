...
;             ObjRef: Obj_New(), $ ; The "self" object reference.
...
IF self.cr_only THEN BEGIN
   IF self.event_func NE "" THEN BEGIN
      thisEvent = {FSC_Field, self.tlb, event.top, 0L, self.theValue, self.dataType, self}
...
;
   ...
   Decimal=decimal, $               ; The number of digits to the right of the decimal
                                    ;   point in FLOATVALUE and DOUBLEVALUE numbers.
...
uvalue = *self.uvalue
value = *self.thevalue           ;<---------HERE  this was Self.Value
Name = Self.Name
...
RETURN, Obj_New("FSC_INPUTFIELD", $
   ...
   Value=value, $                   ; The "value" of the compound widget.
   XSize=xsize,$                    ; The X size of the Text Widget.
   Name = Name)               ;The name pf the object (a scalar string)
END
...
field1 = FSC_INPUTFIELD(tlb, Title='Integer:', LabelSize=50, Digits=4, $
   Value=5, /IntegerValue, UValue='Field 1 Event', Event_Pro='Example_Event', /CR_Only)
field2 = FSC_INPUTFIELD(tlb, Title='Float:', LabelSize=50, Value=45.6, $
   /FloatValue, /CR_Only, UValue='Field 2 Event', Event_Pro='Example_Event', Decimal=2, /Positive)
