...
; PURPOSE:
;
;    The purpose of this function is to plot 2D data (scalar or vector) using
;    object graphics, creating a new widget for the vizualisation of the
;    data. All plot options can be set with the command line. Most options can
;    also be changed interactively using the widget created
;
; AUTHOR:
;
;   Ricardo Fonseca
...
; INPUTS:
;
;    Data2D: For scalar data this is either an two dimensional Byte, Integer,
;       Longword Integer, Floating Point or Double Precision Floating Point
;       array or a pointer to such array containing the data we want to plot.
;       For vector data this must be a two element pointer vector pointing to
;       the two components of the vector field, which must also be of one of
;       the data types mentioned above.  The widget generated needs to keep the
;       data values; if the argument is an array the default behaviour is to
;       copy the data to an internal buffer, but if the argument is a pointer
;       the default behaviour is to only keep a copy of that pointer, sharing
;       the data with other routines. If no argument is present the routine
;       returns quietly.  See also VECTOR, NO_COPY and NO_SHARE
;
; KEYWORDS:
;
;    ADDCONTOUR: Set this keyword to add a contour level plot. See also
;       CONTOURLEVELS and N_LEVELS.
;
;    (XY)AXIS: Set this keyword to an array containing the values for the
;       axis. If the dimensions of this array doesn't match the dimensions of
;       Data2D the routine issues an error message.
;
;    D(XY): Set the resolution of the given axis. Appereantly default values
;       are 1.
;
;    R(XY): You can setup relative center for axis which is number between
;       0. and 1. relative to left lower corner. Defaul values are 0. for
;       both.
;
;    AXISSCALE: Scaling of values on the axis. This is usefull espetialy when
;      your are delaing with axis values relative to the some typical value
;      (i.e. radius of the planet.) See also R(XY) and D(XY)
;
;    BOTTOM: Set this keyword to a three element byte array [R,G,B] containing
;       the color for the bottom side of the surface. If not specified the
;       routine will use either the color specified by COLOR or the vertex
;       colors if VERT_COLORS is set. If both BOTTOM and VERT_COLORS are
;       specified, the routine will draw the top surface using vertex coloring
;       but will respect the BOTTOM color setting. This keyword has no
;       immediate effect if TYPE <> 1 and is ignored quietly, but the value is
;       stored and if you switch to a surface plot using the widget interface
;       it will remember the color specified here. See also COLOR, VERT_COLORS
;       and TYPE.
;
;    COLOR: Set this keyword to a three element byte array [R,G,B] containing
;       the color for the top side of the surface. The default is red
;       [255b,0b,0b]. If BOTTOM was not specified then this color will also be
;       used for the bottom side of the surface. If VERT_COLORS is set this
;       keyword has no effect.  This keyword has no immediate effect if TYPE =
;       0 and is ignored quietly, but the value is stored and if you switch to
;       a surface plot using the widget interface it will remember the color
;       specified here. See also BOTTOM, VERT_COLORS and TYPE.
;
;    CONTOURLEVELS: Set this keyword to a vector containing the isolevels to be
;       used for the contour plot. If this keyword is set the number of levels
;       drawn will be the same as the number of elements in the vector and
;       N_LEVELS will have no effect. If not specified the routine will chose
;       N_LEVELS levels automatically. If ZLOG is not set these values will be
;       evenly spaced over the data values range. If ZLOG is set a logarithmic
;       set of values is chosen. See also ADDCONTOUR, N_LEVELS, ZMIN, ZMAX, and
;       ZLOG.
;
;    CT: Set this keyword to the color table you wish to use for both color
;       mapped and rubber sheet (surface) plots with VERT_COLORS set. The
;       default is 25, Mac Style. See also VERT_COLORS.
;
;    DG: Set this keyword to display the output of the plot as if it was a
;       Direct Graphics routine. The routine will respect the !P.MULTI system
;       variable allowing you to combine multiple plots generated by this or
;       other direct graphics routines. Note that at the moment only screen
;       output is supported and that OPLOT and similar routines will not work,
;       and that system variables are not set according to the plot
;       created. Setting this keyword results in not displaying the
;       widget. However, both the FILEOUT and IMAGE keywords can be used
;       together with this keyword. See Also FILEOUT and IMAGE
;
;    FILEOUT: Set this keyword to the name of the file for saving the plot as a
;       24-bit tiff file.  Setting this keyword results in not displaying the
;       widget. However, both the FILEOUT and DG keyword can be used together
;       with this keyword. See Also IMAGE and DG
;
;    FONTSIZE: Set this keyword to the fontsize (relative to the default
;       fontsize) you wish to use.
;
;    HIDDEN_LINES: Set this keyword if you want to do hidden line removal. This
;       keyword has no immediate effect if TYPE = 0 and is ignored quietly, but
;       the value is stored and if you switch to a surface plot using the
;       widget interface it will remember the hidden line setting specified
;       here. See also TYPE.
;
;    IMAGE: Set this keyword to the variable name for returning the plot as a
;       24-bit image. If the variable cannot be overwritten this keyword will
;       be ignored. Setting this keyword results in not displaying the
;       widget. However, both the FILEOUT and DG keyword can be used together
;       with this keyword. See Also FILEOUT and DG
;
;    INTERP: Setting this keyword results in interpolating the data provided to
;       the screen resolution before plotting.  This keyword has no immediate
;       effect if TYPE <> 0 and is ignored quietly, but the value is stored and
;       if you switch to a color map plot using the widget interface it will
;       remember this setting. See also TYPE.
;
;    INVCOLORS: Set this keyword to use black axis on white background. The
;       default is white axis on black background.
;
;    N_LEVELS: Set this keyword to the number of levels to use for the contour
;       plot. The default is 7.  If CONTOURLEVELS is set this keyword has no
;       effect. See also ADDCONTOUR and CONTOURLEVELS.
;
;    NO_COPY: If Data2D is an array and this keyword is set, rather than
;       copying the data from the input parameter into the internal buffer the
;       routine will take the data away from the input parameter and attach it
;       directly to the internal buffer, resulting in the Data2D argument
;       becoming undefined. This keyword has no effect if Data2D is a pointer
;       and is ignored quietly.  See also NO_SHARE (and NO_COPY keyword under
;       PTR_NEW and WIDGET_CONTROL)
;
;    NO_SHARE: If Data2D is a pointer, rather than copying the pointer from the
;       input parameter into an internal pointer and keeping the data attached
;       to Data2D the routine will also detach the data from the input pointer
;       and then free it. This is effectively the same as doing: plot2D,
;       (*pData), /NO_COPY & ptr_free, pData and is provided only for
;       convinience. This keyword has no effect if Data2D is an array and is
;       ignored quietly. See also NO_COPY
;
;    (XY)RANGE: Set this keyword to the axis range to use. At the moment this
;       feature is limited to color mapped plots.
;
;    RES: Set this keyword to a two element vector containing the plot
;       resolution in pixels. The defaults are [600,400] for TYPE = 0 and
;       [600,600] for TYPE = 1. See also TYPE.
;
;    SHADING: Set this keyword to the lighting styly to use. Available options
;      are:
;               0 - Flat
;               1 - Gouraud (default)
;
;       This keyword has no immediate effect if TYPE = 0 and is ignored
;       quietly, but the value is stored and if you switch to a surface plot
;       using the widget interface it will remember the lighting style
...
;       This keyword has no immediate effect if TYPE = 0 and is ignored
;       quietly, but the value is stored and if you switch to a surface plot
;       using the widget interface it will remember the surface style specified
;       here. See also TYPE.
;
;    SUBTITLE: Set this keyword to a string containing the plot sub-title you
;       wish to use. The default is no title.
;
;    (XYZ)TICKFORMAT: Set this keyword to a standard IDL format string to be
;       used to format the axis tick mark label (see chapter 11 of 'Building
;       IDL Applications' for details on format codes. The default is '', the
;       null string, indicating that IDL will determine the appropriate format
;       for each value.
;
;    TITLE: Set this keyword to a string containing the plot title you wish to
;       use. The default is no title.
;
;    (XYZ)TITLE: Set this keyword to a string containing the axis title you
;       wish to use. The default is no title.
;
;    TRAJCOLOR: Set this keyword to the color you want to use for the plotting
;       of the trajectory in the form [R,G,B]. The default is black
;       [0b,0b,0b]. If TRAJVERT has not been set this keyword has no effect and
;       is ignored quietly. See also TRAJVERT, TRAJPOINTS, TRAJSTYLE and
;       TRAJTHICK
;
;    TRAJPOINTS: Set this keyword to an array of polyline descriptions for the
;       trajectory specified by TRAJVERT. A polyline description is an integer
;       or longword array in the form [n, i0, i1, ..., in-1], where n is the
;       number of vertices that define the polyline and i0 ... in-1 are indices
;       into the vertices array specified in TRAJVERT. If not specified the
;       routine will connect all the points specified in TRAJVERT
;       sequentially. If TRAJVERT has not been set this keyword has no effect
;       and is ignored quietly. See also TRAJVERT, TRAJCOLOR, TRAJSTYLE and
;       TRAJTHICK
;
;    TRAJSTYLE: Set this keyword to the line style you want to use for the
;       plotting of the trajectory. Available options are: 0 - Solid (default)
;       1 - Dotted 2 - Dashed 3 - Dash Dot 4 - Dash Dot Dot Dot 5 - Long Dash 6
;       - No Line Drawn If TRAJVERT has not been set this keyword has no effect
;       and is ignored quietly. See also TRAJVERT, TRAJPOINTS, TRAJCOLOR and
;       TRAJTHICK
;
;    TRAJTHICK: Set this keyword to the thickness of the line you want to use
;       for the plotting of the trajectory from 1 to 10. The default is 1. If
;       TRAJVERT has not been set this keyword has no effect and is ignored
;       quietly.  See also TRAJVERT, TRAJPOINTS, TRAJSTYLE and TRAJCOLOR
;
;    TRAJVERT: Set this keyword to a [2,N] array containing the list of points
;       making up the trajectory to be overplotted. If TRAJPOINTS is not
;       specified the routine will connect all the points sequentially.  If not
;       specified no trajectory is plotted. See also TRAJPOINTS, TRAJCOLOR,
;       TRAJSTYLE and TRAJTHICK
;
;    TYPE: Set this keyword to the plot type to use. Available options are:
;              0 - Color Mapped (default)
;              1 - Rubber Sheet (Surface)
;
;    VECTOR: Setting this keyword results in the routine expecting vector data
;       in the form of a two element pointer vector in Data2D, and then
;       plotting the vector field on top of the field intensity. Note that if
;       you set this switch the routine will only accept this kind of Data2D;
;       the keyword NO_SHARE maintains its functionality and NO_COPY has no
;       effect. See also NO_SHARE.
; 
;    VERT_COLORS: Set this keyword if you want to color the vertex of the
;       surface plot according to the data value at their point using the color
;       table specified by CT. If this keywprd is set it will override the
;       COLOR keyword (but not the BOTTOM keyword). This keyword has no
;       immediate effect if TYPE = 0 and is ignored quietly, but the value is
;       stored and if you switch to a surface plot using the widget interface
;       it will remember the setting specified here. See also CT, COLOR, BOTTOM
;       and TYPE.
;
;    WINDOWTITLE: Set this keyword to a string containing the title for the
;       widget window. The default is 'Plot 2D'.
;
;    ZMIN: Set this keyword to the minimum value to be plotted. If not supplied
;       and ZLOG is no set the routine will autoscale to the minimum value of
;       the data. If not supplied and ZLOG is set then the routine will
;       autoscale to the minimum absolute value different from 0. See also
;       ZLOG.
;
;    ZMAX: Set this keyword to the maximum value to be plotted. If not supplied
;       and ZLOG is no set the routine will autoscale to the maximum value of
;       the data. If not supplied and ZLOG is set then the routine will
;       autoscale to the maximum absolute value. See also ZLOG.
;
;    ZLOG: Set this keyword to use a log scale on the Z (data) axis. The
;       routine will automatically take the absolute value of the data
;       supplieded if this keyowrd is set. Setting this keyword affects the
;       autoscale values. See also ZMIN and ZMAX.
;    
; OUTPUTS:
;
;    The routine generates a widget with the plot of the supplied data. The
;    kewyords (XY)RANGE return the actual ranges used. If IMAGE is set then the
;    routine will not create the widget and will return a 24-bit image with the
;    plot on that variable. if FILEOUT is set the routine will not create a
;    widget and will save a tiff file with a 24-bit image of the plot. If DG is
;    set then the routine does not create a widget and will display the plot
;    using direct graphics.
;
; RESTRICTIONS:
;
;    This routine works only with two dimensional Byte, Integer, Longword
;    Integer, Floating Point or Double Precision Floating Point. If the dataset
;    to plot is very large you should consider either passing it as a pointer
;    or using the NO_COPY keyword
;
; EXAMPLE:
;
...
;
;    Added IMAGE, FILEOUT and DG keywords, 23 Sep 2000 zamb.  Fixed the
;    autoscaling so now it works with uniform plots, added check if DataMin >
;    DataMax, fixed the INVCOLOR switch, corrected title positioning, added
;    INTERP keyword, fixed surface geometry for xres <> yres, 25 Sep 2000 zamb.
;
...
; MAIN:
;---------------------------------------------------------------------
PRO PLT_PLOT2D, _EXTRA=extrakeys, $
                                ; Data to be plotted
            _pData, NO_COPY = no_copy, NO_SHARE = no_share, $
            INFO_STRUCT = sInfo, $
                                ; Data Ranges
            ...
            DX = dx, DY = dy, RX = rx, RY = ry, AXISSCALE = AXISSCALE,  $
            XTITLE = XAxisTitle, YTITLE = YAxisTitle, ZTITLE = ZAxisTitle, $
            XTICKFORMAT = XTickFormat, YTICKFORMAT = YTickFormat, $
            ZTICKFORMAT = ZTickFormat, $
                                ; Plot options
            TYPE = type, $
            STYLE = style, HIDDEN_LINES = hidden_lines, $
            VERT_COLORS = vert_colors, SHADING = shading, $
            COLOR = color, BOTTOM = bottom, FONTSIZE = fontsize, $
            INTERP = interp, $
            ZSCALE_POS = zscale_pos, $
                                ; Color Table Options
            CT = ColorTable, BOTTOM_CT = BottomStretch, $
            TOP_CT = TopStretch, GAMMA_CT = GammaCorrection, $
            ...
            INVCOLORS = iInvColors, $
                                ; Output Options
            IMAGE = imageout, FILEOUT = imagefile, UI = user_interface
...
; ############################################################################
   ...
   ; INVCOLORS
   ;
   ; Invert the colors for the background and the axis and labels. Default is
   ; white on black
   
   if N_Elements(iInvColors) eq 0 then begin
     iInvColors = 1
   end else begin
     if (iInvColors ne 0) then iInvColors = 0
   end    
   
   ; FONTSIZE
   ;
   ; Font size for titles and axis labels as a fraction of the reference
   ; size. Default is 1.0
   ...
   ;
   ; Set this keyword to add a contour plot on top of a color density
   ; plot. This keyword only works with color density plots
   ...
   ; Font selection
   ;
   ; Set this parameter to -1 (default), 0 or 1 to use Hershey, Hardware or
   ; true Type fonts
   ...
   ;
   ; Set these parameters to the string to be used to format the axis tick mark
   ; (IDL Standard)
   ...
   ; X Axis Data
   ;
   ; Use this parameter to provide the X Axis Data. The default is just the
   ; data col number

   if N_Elements(XAxisDataOrg) eq 0 then Begin
         XAxisData = FIndGen(NX)
         if N_Elements(dx) ne 0 then XAxisData = XAxisData*dx
         if N_Elements(rx) ne 0 then XAxisData = XAxisData-max(XAxisData)*rx
         if N_Elements(axisscale) ne 0 then XAxisData = XAxisData/axisscale
   ...
   if N_Elements(YAxisDataOrg) eq 0 then Begin
         ...
         if N_Elements(dy) ne 0 then YAxisData = YAxisData*dy
         if N_Elements(ry) ne 0 then YAxisData = YAxisData-max(YAxisData)*ry
         if N_Elements(axisscale) ne 0 then YAxisData = YAxisData/axisscale
   ...
   if (Keyword_Set(user_interface)) then begin
   ...
   ;
   ; Set this keyword to use a log scale for the Z Axis. The default is to use
   ; a linear scale.
   ...
   ;
   ; Use this parameter to set the minimum value of Z to be plotted. The
   ; default is to use an autoscale value

   ; ZMAX
   ;
   ; Maximum value of z to be plotted
   ;
   ; Use this parameter to set the maximum value of Z to be plotted. The
   ; default is to use an autoscale value
...
;               DataMin = DataMax/(2.0*logScale)
   ...
   if (Arg_Present(imageout) or $
       ...
       not Keyword_Set(user_interface)) then show_widget = 0b $
   else show_widget = 1b
...
; TOP_STRETCHSet=TopStretch)
   ...
   if (N_Elements(BottomStretch) ne 0) then $
      oPalette -> SetProperty, BOTTOM_STRETCH=BottomStretch
   if (N_Elements(TopStretch) ne 0) then $
      oPalette -> SetProperty, TOP_STRETCH = TopStretch
   if (N_Elements(GammaCorrection) ne 0) then $
      oPalette -> SetProperty, GAMMA_CORRECTION = GammaCorrection

   oContainer -> Add, oPalette

   ;------------------------------ Fonts
   ;                               -------------------------------------------
   ;; @@@ Change it according ... for ps output
  ...
  ; oWindow -> SetCurrentCursor, 'CROSSHAIR'
   ...
   sState = { $
     ...
     if not (keyword_set(user_interface)) then begin
