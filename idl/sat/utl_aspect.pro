...
; PURPOSE:
;
;  This function calculates and returns the normalized position coordinates
;  necessary to put a plot with a specified aspect ratio into the currently
;  active graphics window. It works on the display output window as well as in
;  a PostScript output window.
;
; AUTHOR:
;
...
; INPUTS:
;
;  aspectRatio: A floating point value that is the desired aspect ratio (ratio
;     of heigth to width) of the plot in the current graphics output window. If
;     this parameter is missing, an aspect ratio of 1.0 (a square plot) is
;     assumed.
;
; KEYWORD PARAMETERS:
;
;  MARGIN: The margin around the edges of the plot. The value must be a
;     floating point value between 0.0 and 0.5. It is expressed in normalized
;     coordinate units. The default margin is 0.15.
;
;  WINDOWASPECT: The aspect ratio of the target window. If not provided, the
;     value is obtained from the current graphics window.
;
; OUTPUTS:
;
...
; EXAMPLE:
;
;  To create a plot with an aspect ratio of 1:2 and a margin of 0.10 around the
;  edge of the output window, do this:
;
...
FUNCTION UTL_ASPECT, aspectRatio, MARGIN=margin, WindowAspect=wAspectRatio
  ...
  IF (s(s(0)+1) NE 4) AND  (s(s(0)+1) NE 5) THEN $
     MESSAGE, 'Aspect Ratio is not a FLOAT or DOUBLE. Take care...', $
              /Informational

   ; Check for margins.

  IF N_ELEMENTS(margin) EQ 0 THEN margin = 0.15
  IF N_ELEMENTS(margins) EQ 0 OR $
     N_ELEMENTS(margins) NE 4 THEN margins = [margin, margin, margin, margin]
  ...
  IF N_Elements(wAspectRatio) EQ 0 THEN $
