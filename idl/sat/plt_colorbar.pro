...
; KEYWORD PARAMETERS:
;
;       ANNOTATECOLOR: The name of the "annotation color" to use. The names are
;          those for FSC_COLOR, and using the keyword implies that FSC_COLOR is
;          also in your !PATH. If this keyword is used, the annotation color is
;          loaded *after* the color bar is displayed. The color will be
;          represented as theColor = FSC_COLOR(ANNOTATECOLOR, COLOR). This
;          keyword is provide to maintain backward compatibility, but also to
;          solve the problem of and extra line in the color bar when this kind
;          of syntax is used in conjunction with the indexed (DEVICE,
;          DECOMPOSED=0) model is used:
;
;                          LoadCT, 33
;                          TVImage, image
...
;         True-Type: 1.
;
;       FORMAT: The format of the bar annotations. Default is '(F5.1)'.
;
...
;         form as the POSITION keyword on a plot. Default is [0.88, 0.10, 0.95,
;         0.90] for a vertical bar and [0.10, 0.88, 0.90, 0.95] for a
;         horizontal bar.
;
...
;
;       RIGHT: This puts the labels on the right-hand side of a vertical color
;         bar. It applies only to vertical color bars.
;
;       TICKNAMES: A string array of names or values for the tick marks.
;
;       TITLE: This is title for the color bar. The default is to have no
;         title.
;
;       TOP: This puts the labels on top of the bar rather than under it.  The
;         keyword only applies if a horizontal color bar is rendered.
;
...
;       v.0.3.0 2007-03-13 Adoapted as a part of SAT IDL library
;-
;
;###########################################################################
;
...
PRO PLT_COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
   ...
   TICKNAMES=ticknames, REVERSE=reverse, ANNOTATECOLOR=annotatecolor
