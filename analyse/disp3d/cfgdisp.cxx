...
double ConfigDisp::GetThetaSample (int i) const
{
  SAT_ASSERT (0 <= i && i < ThetaSamp ());
  double ds = (ThetaMax() - ThetaMin()) / double(ThetaSamp()-1);
  return ds * double(i) + ThetaMin ();
}
...
double ConfigDisp::Mass (int sp) const
{
  if (_mass[sp] == 0)
...
complex<double> ConfigDisp::Zeta (int sp, int n, double kpar, double kper,
				  complex<double> w) const
{
  complex<double> kuj = kpar * V0Par(sp) + kper * V0Per(sp);
  complex<double> nom = w + double(n) * CycloFreq(sp) - kuj;
  complex<double> den = M_SQRT2 * kpar * VthPar(sp);
  return nom/den;
}

double ConfigDisp::Lambda (int sp, double kper) const
{
  double aj = VthPar(sp) / CycloFreq(sp);
  double lj = kper * aj;
  return Ani(sp) * lj*lj;
}
