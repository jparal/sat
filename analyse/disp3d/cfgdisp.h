...
public:
  ...
  /// @param tht Theta angle [degrees]
  double GetKSamplePar (int i, double tht) const
  { return GetKSample(i) * Math::Cos(tht*M_PI/180.); }
  /// @param tht Theta angle [degrees]
  double GetKSamplePer (int i, double tht) const
  { return GetKSample(i) * Math::Sin(tht*M_PI/180.); }

  /// @return Minimal Theta angle [degrees]
  double ThetaMin () const { return _theta[0]; }
  double ThetaMax () const { return _theta[1]; }
  int ThetaSamp ()   const { return _tsamp; }
  double GetThetaSample (int i) const;
  ...
  double PlasmaFreq2 (int sp) const
  { double wp = PlasmaFreq (sp); return wp*wp; }
  ...
  complex<double> Zeta (int sp, int n, double kpar, double kper,
			complex<double> w) const;

  /// lambda_j = A * (k_per a_j)^2 = T_per/T_par *(k_per v_th,par / Omega_j)^2
  double Lambda (int sp, double kper) const;
